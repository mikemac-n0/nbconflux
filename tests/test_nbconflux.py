...
def notebook_path(request):
    return os.path.join(
        os.path.abspath(os.path.dirname(request.module.__file__)),
        "notebooks",
...
def test_post_to_confluence(notebook_path, page_url, server):
    ...
    server.add(
        "GET",
        "http://confluence.localhost/rest/api/content?title=Some+Page+Name&spaceKey=SPACE",
        match_querystring=True,
        json={"results": [{"id": 12345}]},
    )
    # Mock current page attachment lookup
    server.add(
        "GET",
        "http://confluence.localhost/rest/api/content/12345/child/attachment?expand=version",
        match_querystring=True,
        json={
            "results": [
                {"id": 1, "title": "output_6_0.png", "version": {"number": 5}},
                {"id": 5, "title": "fake-image-2.jpg", "version": {"number": 10}},
            ]
        },
    )
    ...
    server.add(
        "GET",
        "http://confluence.localhost/rest/api/content/12345",
        json={"title": "fake-title", "version": {"number": 100}},
    )
    ...
    server.add(
        "POST",
        "http://confluence.localhost/rest/api/content/12345/child/attachment/1/data",
    )
    server.add(
        "POST", "http://confluence.localhost/rest/api/content/12345/child/attachment"
    )

    html, resources = nbconflux.notebook_to_page(
        notebook_path,
        page_url,
        "fake-username",
        "fake-pass",
    ...
    assert (
        '<ac:image ac:alt="Juputer Logo"><ri:url ri:value="http://jupyter.org/assets/nav_logo.svg"></ri:url></ac:image>'
        in html
    )
    # Image is a properly versioned attachment
    assert (
        '<ac:image><ri:url ri:value="http://confluence.localhost/download/attachments/12345/output_6_0.png?version=6" /></ac:image>'
        in html
    )
    ...
    assert (
        '<a href="http://confluence.localhost/download/attachments/12345/nbconflux-test.ipynb?version=1">nbconflux-test.ipynb</a>'
        in html
    )
...
def test_optional_components(notebook_path, page_url, server):
    ...
    server.add(
        ...
        match_querystring=True,
        json={
            "results": [{"id": 1, "title": "output_6_0.png", "version": {"number": 5}}]
        },
    )
    # Mock current page version lookup
    server.add(
...
def test_post_to_unknown(notebook_path, bad_page_url, server):
    ...
    server.add(
        "GET",
        "http://confluence.localhost/rest/api/content?title=Does+Not+Exist&spaceKey=SPACE",
        match_querystring=True,
        json={"results": []},
    )
    with pytest.raises(ValueError) as ex:
        nbconflux.notebook_to_page(
            notebook_path, bad_page_url, "fake-username", "fake-pass"
        )
