...
class ConfluencePreprocessor(Preprocessor):
    ...
    def preprocess(self, nb, resources):
        ...
        # names and versions.
        path = "/rest/api/content/{page_id}/child/attachment?expand=version".format(
            page_id=self.exporter.page_id
        )
        resources["attachments"] = {}
        while path:
            url = f"{self.exporter.server}{path}"
            resp = requests.get(
                url, auth=(self.exporter.username, self.exporter.password)
            )
            ...
            resources["attachments"].update(
                {
                    result["title"]: Attachment(
                        result["id"], result["version"]["number"], None, None
                    )
        ...
        for filename in to_be_attached:
            try:
                attachment_id, attachment_version, _, _ = resources["attachments"][
                    filename
                ]
            except KeyError:
                ...
                upload_url = (
                    "{server}/rest/api/content/{page_id}/child/attachment".format(
                        server=self.exporter.server, page_id=self.exporter.page_id
                    )
                )
            else:
                upload_url = "{server}/rest/api/content/{page_id}/child/attachment/{attachment_id}/data".format(
                    server=self.exporter.server,
                    page_id=self.exporter.page_id,
                    attachment_id=attachment_id,
                )

            # Populate the download url template
            download_url = "{server}/download/attachments/{page_id}/{filename}?version={version}".format(
                server=self.exporter.server,
                page_id=self.exporter.page_id,
                filename=filename,
                version=attachment_version + 1,
            )
