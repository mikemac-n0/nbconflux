...
class ConfluenceExporter(HTMLExporter):
    ...
    notebook_css = Unicode(
        config=True, default_value=DEFAULT_CSS, help="Path to the CSS"
    )
        ...
        overrides = Config(
            {
                ...
                "HighlightMagicsPreprocessor": {"enabled": False},
                "TemplateExporter": {
    ...
    def get_server_info(self, url):
        ...
        if len(segs) > 2 and segs[1] == "display":
            ...
            resp = requests.get(
                "{server}/rest/api/content?title={title}&spaceKey={space}".format(
                    server=server, title=title, space=space
                ),
    ...
    def update_page(self, page_id, body):
        ...
        resp = requests.get(
            "{server}/rest/api/content/{page_id}".format(
                server=self.server, page_id=page_id
            ),
            auth=(self.username, self.password),
        )
        resp.raise_for_status()
        ...
        resp = requests.put(
            ...
            json={
                ...
                "metadata": {"properties": {"editor": {"value": "v1"}}},
            },
            auth=(self.username, self.password),
        )
        resp.raise_for_status()
    ...
    def add_label(self, page_id, label):
        ...
        resp = requests.post(
            "{server}/rest/api/content/{page_id}/label".format(
                server=self.server, page_id=page_id
            ),
    ...
    def from_notebook_node(self, nb, resources=None, **kw):
        ...
        if self.attach_ipynb:
            with open(self.notebook_filename, encoding="utf-8") as f:
                self.add_or_update_attachment(
                    self.notebook_filename, f.read(), resources
                )
