...
def main(argv=None):
    ...
    parser.add_argument(
        "--exclude-toc", action="store_true", help="Do not generate a table of contents"
    )
    parser.add_argument(
        "--exclude-ipynb",
        action="store_true",
        help="Do not attach the notebook to the page",
    )
    parser.add_argument(
        "--exclude-style",
        action="store_true",
        help="Do not include the Jupyter base stylesheet",
    )
    parser.add_argument(
        "--include-mathjax", action="store_true", help="Enable MathJax on the page"
    )
    parser.add_argument(
        "--extra-labels",
        nargs="+",
        type=str,
        help="Additional labels to add to the page",
    )
    ...
    if username and password:
        print(f"Using credentials for {username} from environment variables")
    ...
    notebook_to_page(
        ...
        username,
        password,
        generate_toc=not args.exclude_toc,
        attach_ipynb=not args.exclude_ipynb,
        enable_style=not args.exclude_style,
        enable_mathjax=args.include_mathjax,
        extra_labels=args.extra_labels,
        notebook_css=notebook_path_css,
    )
